 ...
 *
 * @method string|false __invoke(mixed $data = null) Render the buildable component with provided data and return HTML.
 *
 */
abstract class Buildable {
     ...
     */
    public static function compile(string $pattern, mixed $data = null, string $delimiters = "{{ }}") : int|false {
    ...
    public static function build(string $pattern, iterable $dataset, string $delimiters = "{{ }}") : int|false {
        if (count($dataset) === 0) {
            throw new \InvalidArgumentException("Dataset is empty. No files were built.");
        }
